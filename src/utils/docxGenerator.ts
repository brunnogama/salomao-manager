...
export const generateProposalDocx = async (data: ProposalData, proposalCode: string) => {
    ...
    const docChildren: any[] = [
        ...
        // Section 1: OBJETO ou TEXTO CUSTOMIZADO (SUBSTITUI TUDO ATÉ AS ASSINATURAS SE EXISTIR)
        ...(data.custom_body_text
            ...
            : [
                ...
                // 2.4 Final Success
                ...(data.full_success_clauses && data.full_success_clauses.length > 0 ? (
                    data.full_success_clauses.map((clause: any, idx: number) => {
                        const baseIdx = 3 + (data.intermediate_fees?.length || 0);
                        const isPercent = clause.type === 'percent';
                        const formatter = isPercent ? formatPercent : formatCurrency;

                        return new Paragraph({
                            alignment: AlignmentType.JUSTIFIED,
                            spacing: { line: 288, after: 200 },
                            children: [
                                new TextRun({ text: `2.${baseIdx + idx}.	Honorários finais de êxito de `, font: standardFont, size: 22 }),
                                new TextRun({ text: formatter(clause.value), font: standardFont, size: 22, bold: true }),
                                new TextRun({ text: getExtensoText(clause.value, isPercent), font: standardFont, size: 22 }),
                                new TextRun({ text: `, ${clause.description}`, font: standardFont, size: 22 }),
                            ]
                        });
                    })
                ) : [
                    new Paragraph({
                        ...
                        spacing: { line: 288, after: 200 },
                        children: [
                            new TextRun({ text: `2.3.	Honorários finais de êxito de `, font: standardFont, size: 22 }),
                            ...
                            new TextRun({ text: `, ${data.final_success_fee_clause || "[incluir texto]"}`, font: standardFont, size: 22 }),
                        ]
                    })
                ]),
