...
export function FamiliaFormModal({ isOpen, onClose, onSave, initialData }: FamiliaFormModalProps) {
    ...
    return (
      <div className="space-y-1 flex flex-col h-full relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div 
          ...
          className={`w-full h-[38px] px-3 flex items-center justify-between bg-gray-50/50 border rounded-xl text-xs transition-all cursor-pointer group ${
        ...
        >
          ...
          <div className="flex items-center gap-1">
             <button 
              ...
              className="p-1 hover:bg-blue-50 text-gray-300 hover:text-blue-600 rounded-lg transition-all"
            >
        ...
        {isDropdownOpen && (
          <div className="absolute top-[45px] left-0 w-full bg-white border border-gray-100 shadow-2xl rounded-xl z-[100] overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200">
            <div className="max-h-[180px] overflow-y-auto py-1 custom-scrollbar">
              {optionsList.map((opt: string) => (
                <div 
                  ...
                  className={`px-3 py-2 text-xs font-medium flex items-center justify-between transition-colors cursor-pointer ${
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-2 bg-[#0a192f]/60 backdrop-blur-md transition-all">
      <div className="bg-white w-full max-w-6xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[98vh] border border-white/20">
        
        <div className="px-8 py-4 border-b border-gray-100 flex justify-between items-center bg-white flex-shrink-0">
          <div>
            <h3 className="text-xl font-black text-[#112240] tracking-tight">{initialData ? 'Editar Lançamento' : 'Novo Lançamento'}</h3>
            <p className="text-[10px] text-gray-400 font-medium uppercase tracking-widest mt-0.5">Gestão Financeira • Família Salomão</p>
          </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }} className="p-8 overflow-y-auto flex flex-col gap-5 text-[#112240] custom-scrollbar">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="space-y-1 flex flex-col h-full">
                ...
                <input type="date" name="vencimento" value={formData.vencimento} onChange={(e) => setFormData({...formData, vencimento: e.target.value})} className="w-full h-[38px] px-3 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium" required />
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="space-y-1 flex flex-col h-full">
                ...
                <input type="number" step="0.01" name="valor" value={formData.valor} onChange={(e) => setFormData({...formData, valor: e.target.value})} className="w-full h-[38px] px-3 bg-blue-50/30 border border-blue-100 rounded-xl text-xs font-bold text-blue-700 outline-none focus:ring-2 focus:ring-blue-500/20" required />
              </div>
              ...
              <div className="space-y-1 flex flex-col h-full">
                ...
                <button type="button" onClick={() => setIsRateioModalOpen(true)} className="w-full h-[38px] flex items-center justify-between px-3 bg-white border border-dashed border-gray-200 text-gray-600 rounded-xl text-[11px] font-semibold hover:border-blue-400 transition-all">
              ...
              <div className="space-y-1 flex flex-col h-full">
                ...
                <input type="date" name="data_envio" value={formData.data_envio} onChange={(e) => setFormData({...formData, data_envio: e.target.value})} className="w-full h-[38px] px-3 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="space-y-1"><label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest px-1">NF</label><input name="nota_fiscal" value={formData.nota_fiscal} onChange={(e) => setFormData({...formData, nota_fiscal: e.target.value})} className="w-full h-[38px] px-3 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none" /></div>
              <div className="space-y-1"><label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest px-1">Fatura</label><input name="fatura" value={formData.fatura} onChange={(e) => setFormData({...formData, fatura: e.target.value})} className="w-full h-[38px] px-3 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none" /></div>
              <div className="space-y-1"><label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest px-1">Recibo</label><input name="recibo" value={formData.recibo} onChange={(e) => setFormData({...formData, recibo: e.target.value})} className="w-full h-[38px] px-3 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none" /></div>
              <div className="space-y-1"><label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest px-1">Boleto</label><input name="boleto" value={formData.boleto} onChange={(e) => setFormData({...formData, boleto: e.target.value})} className="w-full h-[38px] px-3 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none" /></div>
              <div className="space-y-1"><label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest px-1">O.S.</label><input name="os" value={formData.os} onChange={(e) => setFormData({...formData, os: e.target.value})} className="w-full h-[38px] px-3 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none" /></div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
              <div className="md:col-span-2 space-y-1">
                ...
                <input name="descricao_servico" value={formData.descricao_servico} onChange={(e) => setFormData({...formData, descricao_servico: e.target.value})} className="w-full h-[38px] px-4 bg-gray-50/50 border border-gray-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-blue-500/20 font-medium" placeholder="Resumo do serviço..." />
              </div>
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest px-1">Comprovante (GED)</label>
                <label className="flex items-center gap-2 w-full h-[38px] px-3 border-2 border-dashed border-blue-100 rounded-xl cursor-pointer bg-blue-50/30 hover:bg-blue-50 transition-all">
          ...
          <div className="flex justify-end items-center gap-6 pt-4 border-t border-gray-50 flex-shrink-0">
            <button type="button" onClick={onClose} className="text-[10px] font-black text-gray-400 hover:text-gray-600 transition-all uppercase tracking-widest">Cancelar</button>
            <button type="submit" className="flex items-center gap-2 px-10 py-3 bg-[#1e3a8a] text-white text-[10px] font-black rounded-xl hover:bg-[#112240] shadow-lg transition-all active:scale-95 uppercase tracking-widest">
            ...
            <div className="p-6 space-y-4">
              {editingItem ? (
                <div className="flex gap-2">
                  <input type="text" value={editingItem.current} onChange={(e) => setEditingItem({...editingItem, current: e.target.value})} className="flex-1 p-2 bg-blue-50 border border-blue-200 rounded-lg text-xs outline-none font-bold text-blue-700" />
                  <button type="button" onClick={handleUpdateItem} className="p-2 bg-blue-600 text-white rounded-lg"><Save className="w-4 h-4" /></button>
                </div>
              ) : (
                <div className="flex gap-2">
                  <input type="text" value={newItemValue} onChange={(e) => setNewItemValue(e.target.value)} placeholder="Novo..." className="flex-1 p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs outline-none" />
                  <button type="button" onClick={handleAddItem} className="p-2 bg-blue-600 text-white rounded-lg"><Plus className="w-4 h-4" /></button>
                </div>
              )}
              <div className="max-h-[200px] overflow-y-auto space-y-1 pr-1 custom-scrollbar">
                {options[isManageModalOpen.field]?.map((item) => (
                  <div key={item} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg border border-gray-100 group hover:border-blue-200 transition-all">
      ...
      {isRateioModalOpen && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-[#0a192f]/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-[1.5rem] shadow-2xl p-6 border border-gray-100 animate-in zoom-in duration-200">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <div className="relative">
                  <select value={formData.rateio} onChange={(e) => setFormData({...formData, rateio: e.target.value})} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs outline-none appearance-none font-medium">
              ...
              <div className="space-y-1">
                ...
                <input type="number" value={formData.rateio_porcentagem} onChange={(e) => setFormData({...formData, rateio_porcentagem: e.target.value})} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-black outline-none" />
              </div>
