...
export function AeronaveFormModal({ 
  ...
  const emptyForm: Partial<AeronaveLancamento> = {
    origem: origem,
    tripulacao: '',
    aeronave: 'Aeronave Principal', // Default
    data_missao: '',
    id_missao: undefined,
    nome_missao: '',
  ...
  // Atualiza o form quando abre ou troca de item
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
  ...
  // Máscara de Moeda (Visual apenas, o valor real é number)
  const CurrencyInput = ({ 
    value, 
    onChange, 
    label,
    required = false 
  }: { value: number | undefined, onChange: (val: number) => void, label: string, required?: boolean }) => {
  ...
  const tiposOpcoes = ['Alimentação', 'Combustível', 'Hangaragem', 'Tarifas', 'Hospedagem', 'Manutenção', 'Salários', 'Serviços', 'Outros']
  const fornecedoresOpcoes = ['BR Aviation', 'Shell Aero', 'Infraero', 'Aeroporto SBJR', 'Piloto Chefe', 'Catering Deluxe', 'Oficina Autorizada']
  const docFiscalOpcoes = ['Nota Fiscal', 'Recibo', 'Invoice', 'Cupom Fiscal', 'Boleto']
      ...
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            {origem === 'missao' && (
              <CurrencyInput 
                label="Faturado CNPJ Salomão" 
                value={formData.faturado_cnpj || 0} 
                onChange={val => handleChange('faturado_cnpj', val)} 
              />
            )}
            ...
            <CurrencyInput 
              ...
              value={formData.valor_previsto || 0} 
              onChange={val => handleChange('valor_previsto', val)} 
            />
            ...
            <CurrencyInput 
              ...
              onChange={val => handleChange('valor_pago', val)} 
            />
            ...
            <CurrencyInput 
              ...
              value={formData.valor_total_doc || 0} 
              onChange={val => handleChange('valor_total_doc', val)} 
            />
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
          <button 
            onClick={onClose}
            disabled={loading}
            className="px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest text-gray-500 hover:text-gray-700 hover:bg-gray-200/50 transition-all"
          >
            Cancelar
          </button>
          ...
          <button 
            ...
            disabled={loading}
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest bg-white border border-[#1e3a8a] text-[#1e3a8a] hover:bg-blue-50 transition-all shadow-sm"
          >
          ...
          <button 
            ...
            disabled={loading}
            className="flex items-center gap-2 px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest bg-[#1e3a8a] text-white hover:bg-[#112240] transition-all shadow-lg active:scale-95"
          >
