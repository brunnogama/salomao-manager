...
export function CertificateNameManagerModal({ isOpen, onClose, onUpdate }: Props) {
    ...
    const fetchNames = async () => {
        setLoading(true);
        const { data } = await supabase.from('certificate_names').select('*').order('name');
    ...
    const handleAdd = async () => {
        ...
        if (names.some(n => n.name.toLowerCase() === newName.trim().toLowerCase())) {
            toast.error('Este nome de certidão já existe!');
            return;
        }

        setLoading(true);
        const { error } = await supabase.from('certificate_names').insert([{
            name: newName.trim(),
            status: 'active'
        }]);
        ...
        if (!error) {
            ...
            toast.error('Erro ao adicionar nome: ' + error.message);
        }
        setLoading(false);
    };
    ...
    const toggleStatus = async (item: any) => {
        ...
        const { error } = await supabase.from('certificate_names').update({ status: newStatus }).eq('id', item.id);
        ...
        <div className="fixed inset-0 bg-[#0a192f]/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300 border border-gray-100 flex flex-col max-h-[90vh]">
                ...
                <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 flex-shrink-0">
                    <h3 className="text-lg font-black text-[#0a192f] uppercase tracking-tight">Gerenciar Nomes</h3>
                    ...
                    <div className="space-y-2 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        {filteredNames.length > 0 ? (
                            ...
                            <div className="py-8 text-center flex flex-col items-center justify-center h-full">
                                ...
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Nenhum nome cadastrado</p>
                            </div>
                        )}
                    </div>
                </div>
