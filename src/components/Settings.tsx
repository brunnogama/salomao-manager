...
export function Settings() {
  ...
  const handleDeleteSocio = async (name: string) => {
    if (confirm(`Remover vínculo do sócio "${name}"?`)) {
        setLoadingSocios(true)
  ...
  const handleSystemReset = async () => {
    ...
    if (confirm('PERIGO: Isso apagará TODOS os dados (Clientes e Magistrados). Tem certeza?')) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true)
    setStatus({ type: null, message: 'Processando...' })
    ...
    <div className="max-w-6xl mx-auto pb-12 space-y-8 relative">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-orange-200 p-6">
        <div className="flex items-center gap-3 mb-6">
            ...
            <div><h3 className="font-bold text-[#112240] text-lg">Segurança: Magistrados</h3><p className="text-sm text-gray-500">Controle de acesso.</p></div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">PIN (4 dígitos)</label>
                <input type="text" maxLength={4} value={magistradosConfig.pin} onChange={e => setMagistradosConfig({...magistradosConfig, pin: e.target.value.replace(/\D/g,'')})} className="w-full border border-gray-300 rounded-lg p-2.5 outline-none focus:border-orange-500 font-mono text-center tracking-widest text-lg" placeholder="0000" />
            </div>
            <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Emails Permitidos</label>
                <textarea rows={3} value={magistradosConfig.emails} onChange={e => setMagistradosConfig({...magistradosConfig, emails: e.target.value})} className="w-full border border-gray-300 rounded-lg p-2.5 outline-none focus:border-orange-500 text-xs" placeholder="email@exemplo.com" />
            </div>
        </div>
        <div className="mt-4 flex justify-end">
            <button onClick={handleSaveConfigMagistrados} disabled={loadingConfig} className="px-4 py-2 bg-orange-600 text-white font-bold rounded-lg hover:bg-orange-700 transition-colors flex items-center gap-2"><Save className="h-4 w-4" /> Salvar Segurança</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => openUserModal()} className="flex items-center gap-2 text-sm bg-blue-50 text-blue-700 px-3 py-2 rounded-lg font-bold hover:bg-blue-100"><UserPlus className="h-4 w-4" /> Novo</button>
          </div>
          <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                  <thead><tr className="border-b border-gray-100 text-xs text-gray-400 uppercase"><th className="py-3 pl-2">Nome</th><th>Email</th><th>Cargo</th><th>Status</th><th className="text-right pr-2">Ações</th></tr></thead>
                  <tbody className="text-sm">
                      {loadingUsers ? <tr><td colSpan={5} className="py-4 text-center">Carregando...</td></tr> : users.map(user => (
                          <tr key={user.id} className="border-b border-gray-50 hover:bg-gray-50">
                              <td className="py-3 pl-2 font-bold">{user.nome}</td>
                              <td className="py-3 text-gray-600">{user.email}</td>
                              <td className="py-3"><span className="px-2 py-0.5 bg-gray-100 rounded text-xs font-bold">{user.cargo}</span></td>
                              <td className="py-3">{user.ativo ? <span className="text-green-600 font-bold flex gap-1"><Check className="h-3 w-3"/> Ativo</span> : <span className="text-red-500 font-bold flex gap-1"><Ban className="h-3 w-3"/> Bloqueado</span>}</td>
                              <td className="py-3 text-right flex justify-end gap-2">
                                  <button onClick={() => handleToggleActive(user)} className="p-1.5 hover:bg-gray-200 rounded"><Shield className="h-4 w-4" /></button>
                                  <button onClick={() => openUserModal(user)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded"><Pencil className="h-4 w-4" /></button>
                                  <button onClick={() => handleDeleteUser(user)} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="h-4 w-4" /></button>
                              </td>
                          </tr>
                      ))}
                  </tbody>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="max-h-64 overflow-y-auto custom-scrollbar pr-2 space-y-2">
                {loadingSocios ? <p className="text-center py-4">Carregando...</p> : sociosStats.map((socio) => (
                    <div key={socio.nome} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg group hover:bg-gray-100">
                        <div className="flex items-center gap-3 min-w-0">
                            <div className="h-8 w-8 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center font-bold text-xs shrink-0">{socio.nome.charAt(0)}</div>
                            <div className="min-w-0">
                                {editingSocio === socio.nome ? (
                                    <input autoFocus className="text-sm font-bold bg-white border border-purple-300 rounded px-2 py-0.5 w-full outline-none" defaultValue={socio.nome} onBlur={() => handleUpdateSocio(socio.nome)} onKeyDown={(e) => { if(e.key==='Enter') { setNewSocioName(e.currentTarget.value); handleUpdateSocio(socio.nome); }}} onChange={(e) => setNewSocioName(e.target.value)} />
                                ) : (
                                    <>
                                        <p className="text-sm font-bold text-gray-800 truncate">{socio.nome}</p>
                                        <p className="text-xs text-gray-500">{socio.count} clientes</p>
                                    </>
                                )}
                            </div>
                        </div>
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => { setEditingSocio(socio.nome); setNewSocioName(socio.nome); }} className="p-1.5 text-gray-500 hover:text-purple-600"><Pencil className="h-3.5 w-3.5" /></button>
                            <button onClick={() => handleDeleteSocio(socio.nome)} className="p-1.5 text-gray-500 hover:text-red-600"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                    </div>
                ))}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col">
            ...
            <div className="flex-1 flex flex-col justify-center space-y-4">
                <button onClick={handleDownloadTemplate} className="flex items-center justify-center gap-2 w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-green-500 hover:text-green-600 hover:bg-green-50 transition-all font-medium text-sm">
                    <Download className="h-4 w-4" /> Baixar Planilha Modelo
                </button>
                <div className="relative">
                    <input ref={fileInputRef} type="file" accept=".xlsx, .xls" onChange={handleFileUpload} disabled={loading} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
      ...
      <div className="bg-red-50 rounded-xl border border-red-100 p-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
              ...
              <div><h3 className="font-bold text-red-900">Zona de Perigo</h3><p className="text-xs text-red-700/80">Ações irreversíveis.</p></div>
          </div>
          <button onClick={handleSystemReset} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-lg shadow-sm transition-colors flex items-center gap-2 whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8 border-t border-gray-200">
        <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2 text-[#112240]"><img src="/logo-salomao.png" alt="Salomão" className="h-8 w-auto opacity-80" /></div>
            <p className="text-xs text-gray-500 leading-relaxed max-w-xs">Sistema de gestão desenvolvido exclusivamente para Salomão Advogados.</p>
            <div className="flex items-center gap-1 text-[10px] text-gray-400 font-medium"><Copyright className="h-3 w-3" /> 2024-2026</div>
            <div className="flex items-center gap-1.5 text-[10px] text-gray-400 font-medium"><Code className="h-3 w-3" /><span>Desenvolvido por <strong>Brunno Gama</strong></span></div>
        </div>
        <div className="md:col-span-2 bg-gray-50 rounded-xl p-6 border border-gray-100">
            <div className="flex items-center gap-2 mb-6"><History className="h-4 w-4 text-gray-400" /><h3 className="font-bold text-[#112240] text-lg">Histórico de Versões</h3></div>
            <div className="space-y-8 relative before:absolute before:left-2.5 before:top-2 before:h-full before:w-0.5 before:bg-gray-100">
                {changelog.map((log) => (
                    <div key={log.version} className="relative pl-10">
                        ...
                        <ul className="space-y-1">{log.items.map((item, idx) => (<li key={idx} className="text-xs text-gray-500 flex items-start gap-2"><span className="mt-1.5 h-1 w-1 rounded-full bg-gray-400 flex-shrink-0"></span>{item}</li>))}</ul>
                    </div>
                ))}
            </div>
        </div>
