              ...
              <table className="w-full min-w-[1000px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    <>
                      {/* PRÉ-ADMISSÃO - AÇÕES PENDENTES (Fura filtro) */}
                      {(() => {
                        const today = new Date();
                        today.setHours(0, 0, 0, 0);
                        
                        const actionables = baseFiltered.filter(c => {
                          if (c.status !== 'Pré-admissão') return false;
                          const isDateReached = c.hire_date ? new Date(c.hire_date + 'T00:00:00') <= today : false;
                          return isDateReached;
                        });

                        const awaitings = baseFiltered.filter(c => {
                          if (c.status !== 'Pré-admissão') return false;
                          const isDateReached = c.hire_date ? new Date(c.hire_date + 'T00:00:00') <= today : false;
                          return !isDateReached;
                        });
                        ...
                        return (
                          <>
                            {/* LINHAS FIXAS (Ações Pendentes) */}
                            {actionables.map(c => (
                              <tr key={c.id} onClick={() => handleRowClick(c)} className="bg-orange-100 hover:bg-orange-200/50 cursor-pointer transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] border bg-orange-50 text-orange-700 border-orange-200 whitespace-nowrap">
                                ...
                                <td className="px-6 py-4 text-right">
                                  <div className="flex flex-col items-end gap-1.5" onClick={e => e.stopPropagation()}>
                                    <span className="text-[10px] font-bold text-orange-800 uppercase tracking-wider bg-white/50 px-2 py-0.5 rounded border border-orange-200 whitespace-nowrap">Alterar status para Ativo?</span>
                            ...
                            {/* PRÉ-ADMISSÃO GAVETA (Aguardando Data) */}
                            {awaitings.length > 0 && (
                                ...
                                <td colSpan={7} className="px-6 py-3 border-y border-orange-200">
                                  <div className="flex items-center justify-between">
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-800 flex items-center gap-2">
                                      ...
                                      <span className="bg-orange-200 text-orange-800 px-2 py-0.5 rounded-full text-[9px]">{awaitings.length}</span>
                                    </p>
                            ...
                            {(isPreAdmissaoExpanded || filterStatus.includes('Pré-admissão')) && awaitings.map(c => (
                              <tr key={c.id} onClick={() => handleRowClick(c)} className="bg-orange-50/30 hover:bg-orange-50/60 cursor-pointer transition-colors group">
                                <td className="px-6 py-4 w-12 text-center" onClick={(e) => e.stopPropagation()}>
                                  <input
                                    type="checkbox"
                                    className="w-4 h-4 rounded text-orange-600 border-orange-300 focus:ring-orange-600 cursor-pointer"
                      ...
                      {filtered.filter(c => c.status === 'Pré-Cadastro').map((c) => (
                        <tr key={c.id} onClick={() => handleRowClick(c)} className="bg-amber-50/30 hover:bg-amber-50/60 cursor-pointer transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] border bg-amber-100 text-amber-800 border-amber-300 whitespace-nowrap">
                      ...
                      {filtered.filter(c => c.status === 'active').map((c) => (
                        <tr key={c.id} onClick={() => handleRowClick(c)} className="hover:bg-blue-50/30 cursor-pointer transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] border bg-green-50 text-green-700 border-green-200 whitespace-nowrap">
                      ...
                      {/* INATIVOS */}
                      {filtered.some(c => c.status !== 'active' && c.status !== 'Pré-Cadastro' && c.status !== 'Pré-admissão') && (
                      ...
                      {filtered.filter(c => c.status !== 'active' && c.status !== 'Pré-Cadastro' && c.status !== 'Pré-admissão').map(c => (
                        <tr key={c.id} onClick={() => handleRowClick(c)} className="hover:bg-red-50/10 cursor-pointer transition-colors group grayscale hover:grayscale-0 opacity-70 hover:opacity-100">
                          ...
                          <td className="px-6 py-4">
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] border bg-red-50 text-red-700 border-red-200 whitespace-nowrap">
