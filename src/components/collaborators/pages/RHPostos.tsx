...
export function RHPostos() {
  ...
  const { colaboradores, roles } = useColaboradores();
  ...
  const displayPostos = useMemo(() => {
    return postos.map(posto => {
      let calcOcupados = posto.ocupados;

      let count = 0;
      if (posto.cargo === 'JUNIOR') {
        count = activeColaboradores.filter(c => rolesMap[String(c.role)]?.includes('Advogado Júnior')).length;
        calcOcupados = count;
      } else if (posto.cargo === 'PLENO') {
        count = activeColaboradores.filter(c => rolesMap[String(c.role)]?.includes('Advogado Pleno')).length;
        calcOcupados = count;
      } else if (posto.cargo === 'SENIOR') {
        count = activeColaboradores.filter(c => rolesMap[String(c.role)]?.includes('Advogado Sênior')).length;
        calcOcupados = count;
      } else if (posto.cargo === 'SÓCIO') {
        count = activeColaboradores.filter(c => rolesMap[String(c.role)]?.toLowerCase().includes('sócio')).length;
        calcOcupados = count;
      } else if (posto.cargo === 'CONSULTOR') {
        count = activeColaboradores.filter(c => rolesMap[String(c.role)]?.toLowerCase().includes('consultor')).length;
        calcOcupados = count;
      } else if (posto.cargo === 'ESTAG' || posto.cargo === 'ESTAGIÁRIOS') {
        count = activeColaboradores.filter(c => rolesMap[String(c.role)]?.toLowerCase().includes('estagiário')).length;
        calcOcupados = count;
      } else if (posto.cargo === 'ADM*' || posto.cargo === 'ADMINISTRATIVO') {
        count = activeColaboradores.filter(c => getSegment(c) === 'Administrativo').length;
        calcOcupados = count;
      }

      return {
        ...posto,
        ocupados: calcOcupados
      };
    });
  }, [postos, activeColaboradores, rolesMap]);

  const totals = displayPostos.reduce((acc, current) => ({
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-br from-gray-50 to-gray-100 space-y-6 relative p-4 sm:p-6 pb-20">
      {/* PAGE HEADER */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group">
        ...
        <div className="flex items-center gap-5 relative z-10">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-gray-500 tracking-wide">
              Controle de painel de estações de trabalho disponíveis e ocupadas
            </p>
          </div>
        </div>
      ...
      {/* TABLE CONTENT */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col mx-auto w-full max-w-6xl">
        <div className="overflow-x-auto w-full">
              ...
              {displayPostos.map((posto) => {
                ...
                return (
                  <tr key={posto.id} className="hover:bg-blue-50/50 transition-colors group">
                    ...
                    <td className="py-3 px-4 border-r border-gray-100/50 align-middle text-center">
                      <span className="text-xl font-black text-[#1e3a8a]">{posto.ocupados}</span>
                    </td>
                    <td className="py-3 px-6 border-r border-gray-100/50 text-center align-middle">
                      <span className={`text-xl font-black ${disponiveis < 0 ? 'text-red-500' : disponiveis === 0 ? 'text-gray-400' : 'text-green-600'}`}>
                        {disponiveis}
                      </span>
                    </td>
            ...
            {postos.length > 0 && (
              <tfoot>
                <tr className="bg-yellow-300 border-t-2 border-gray-300">
                  <td className="py-4 px-6 font-black text-red-600 text-lg uppercase text-center border-r border-yellow-400/50">Total</td>
          ...
          {postos.length === 0 && !loading && (
            <div className="p-10 text-center flex flex-col items-center">
              ...
              <p className="text-xs text-gray-400 mt-1">Execute o script do banco de dados para popular os postos.</p>
            </div>
          )}
        </div>
      </div>
